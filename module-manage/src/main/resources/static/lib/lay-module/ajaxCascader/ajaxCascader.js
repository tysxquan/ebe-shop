 ...
 layui.define(["jquery"],(exports)=>{
 	...
 	Private.prototype.init = function(options){
 		...
 		if (param.clear) {
 			...
 			clearButtonDom = `<i class="layui-icon layui-icon-close cascader-clear" style="top:`+((this.param.height/2)-2)+`px;"></i>`
 		}
 		// 渲染主dom
 		store.cascaderDom.after(`
			<div class="cascader-all ` + param.className + ` `+ clearInput +`">
				<input type="text" lay-verify="required" class="layui-input cascader-input" placeholder="`+param.placeholder+`" readonly style="height:`+this.param.height+`px;">
				<i class="layui-icon layui-icon-down cascader-i" style="top:`+((this.param.height/2)-2)+`px;"></i>
				` + clearButtonDom + `
 	...
 	Private.prototype.getChooseData = function() {
 		...
 		for(let i in chooseDom){
 			if(chooseDom[i].innerText){
				...
				contentData.push(chooseDom[i].innerText);
		...
		this.store.contentData = contentData;
	...
	let dom_num = 0;
 	...
 	let cascader = {
		...
		//获取页面中选中的文字内容数据，数组形式
		getContentData:function(elem){
			let obj = this.elemCheck(elem);
			return obj.store.contentData;
		},
