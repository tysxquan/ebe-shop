layui.define('layer',function (exports) {
    ...
    'use strict';
    layui.link(layui.cache.base + "/selectY/selectY.css");
    var P = function(o){
        this.c = {
        ...
        this.createChild = function(items){
            ...
            for(var i = 0; i < items.length; i++){
                var row = o.getChild(o.data,items[i][key.idKey]);
                row = row.length != 0 ? 'child-row' : '';
                var off = items[i].off ? 'layui-disabled' : '';
                ul += '<li class ="'+row+' '+off+'" data-id="'+items[i][key.idKey]+'">'+items[i][key.name]+'</li>'
            }
            ul += '</ul>';
            return ul;
        };
        this.setValue = function (value) {
            var o = this, data = o.data, itemID = [],key = this.c.key;
            for(var i=0;i<data.length;i++){
                for(var j=0;j<value.length;j++){
                    if(value[j] == data[i][key.idKey]){
                        //data[i].checked = true;
                        itemID.push(data[i]);
                    }
                }
    ...
    P.prototype.render = function(){
        var o = this, c = o.c, e = c.elem,key=c.key;
        ...
        o.showPopup(o.data,parseInt(c.key.rootPId));
        ...
        if($(e).val()!==''){
            ...
            for(var i = 0; i < setID.length; i ++){
                o.showPopup(o.data,setID[i][key.idKey]);
                var fistUi = $(e).parent().find('.pop ul');
                fistUi.eq(i).find('li[data-id='+setID[i][key.idKey]+']').addClass('active');
                showValue.push(setID[i][key.name]);
            }
            //显示值
            $(e).parent().find('.show').html(showValue.join('<i>/</i>'));
        }
        ...
        P.prototype.refresh = function() {
            $(e).val("");
            $(e).parent().find('.show').html(c.placeholder);
            $(e).parent().find('.pop .active').removeClass('active');
        };
        P.prototype.getDate = function() {
            var activeLi = $(e).parent().find('.pop li.active'),ids=[],value=[];

            if (activeLi !== undefined) {
                for(var i = 0; i < activeLi.length; i ++){
                    value.push(activeLi.eq(i).html());
                    ids.push(activeLi.eq(i).data('id'));
                }
                return {ids:ids,data: value.join('/')};
            }
